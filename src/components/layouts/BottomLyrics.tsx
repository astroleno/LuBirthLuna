...
export default function BottomLyrics(props: BottomLyricsProps) {
  ...
  // 获取歌词行的基础属性（固定模式）- 与TopLyrics保持一致
  ...
  }, [backLines.map(l => `${l.order}:${l.text}`).join(',')]);
    ...
    <>
      {Array.from(animatedLines.entries()).map(([key, lineData]) => {
        ...
        return (
          <Text
            key={key}
            position={[lineData.align === 'left' ? -1.2 : 1.2, yPosition, -1.2]} // 后层位置
            fontSize={fontSize}
